---
title: "N. Korea Nuclear Iv-Satellite Launch (2009-2009) [#459]"
...
The first high coverage, recall, and precision international event dataset
...
```{r, echo=F, results='hide',  include=T, message=F, cache=F, warning=F}
  ...
  save_file_p <- paste0(here::here(), "//images/ggplots/p_icbe_plot_",crisis,".Rds") #This one has to be pulled back one level because it gets nuked every time you 
  save_file_png <- paste0(here::here(), "/images/website_images/p_icbe_plot_",crisis,".svg") #And then you have to recreate those images every time uhg
  ...
  save_file_mids_p <- paste0(here::here(), "//images/ggplots/p_mids_plot_",crisis,".Rds")
  save_file_mids_png <- paste0(here::here(), "/images/website_images/p_mids_plot_",crisis,".svg")

  save_file_icb_p <- paste0(here::here(), "//images/ggplots/p_icb_plot_",crisis,".Rds")
  save_file_icb_png <- paste0(here::here(), "/images/website_images/p_icb_plot_",crisis,".svg")

  save_file_phoenix_p <- paste0(here::here(), "//images/ggplots/p_phoenix_plot_",crisis,".Rds")
  save_file_phoenix_png <- paste0(here::here(), "/images/website_images/p_phoenix_plot_",crisis,".svg")

  save_file_ucdp_p <- paste0(here::here(), "//images/ggplots/p_ucdp_plot_",crisis,".Rds")
  save_file_ucdp_png <- paste0(here::here(), "/images/website_images/p_ucdp_plot_",crisis,".svg")

  save_file_terrier_p <- paste0(here::here(), "///images/ggplots/p_terrier_plot_",crisis,".Rds")
  save_file_terrier_png <- paste0(here::here(), "/images/website_images/p_terrier_plot_",crisis,".svg")
  
  save_file_icews_p <- paste0(here::here(), "//images/ggplots/p_icews_plot_",crisis,".Rds")
  save_file_icews_png <- paste0(here::here(), "/images/website_images/p_icews_plot_",crisis,".svg")
  
  save_file_mids_incidents_p <- paste0(here::here(), "//images/ggplots/p_mids_incidents_plot_",crisis,".Rds")
  save_file_mids_incidents_png <- paste0(here::here(), "/images/website_images/p_mids_incidents_plot_",crisis,".svg")
...
#ICBe Dyadic Event
if(file.exists(save_file_p) & (fromscratch | !file.exists(save_file_png))    ) {
  p_plot <- readRDS(save_file_p)
  plot_height <- p_plot$sentence_count
  p_width=max(length(p_plot$actor_colors)*1.5,14)
  p_height=max(p_plot$sentence_count/1.75,6)
  ...
  caption=paste0("Source: CrisisEvents.org (Douglass et al. 2022)
                 ...
                 ", p_plot$labels$caption) #tag original to bottom
...
if(fromscratch  | !file.exists(save_file_ft_png)){
  ...
  ft_full_case %>%
  #set_header_labels(values = list(S = "S",
  #                                ICB = "Sentence",
  #                                ICBe = "ICBe Codings (Partial)")
  #                   ) %>%
...
if(file.exists(save_file_icb_p)){
  ...
  if(fromscratch | !file.exists(save_file_icb_png) ){
    p_icb_plot <- readRDS(save_file_icb_p)
    plot_icb_height <- p_icb_plot$sentence_count
    p_width=max(length(p_icb_plot$actor_colors)*1.5,14)
    p_height=min(max(p_icb_plot$sentence_count/2,8),12) #cap it at 49 inches long
    ...
    caption=paste0("Source: CrisisEvents.org (Douglass et al. 2022)
                 ...
                 ", p_icb_plot$labels$caption) #tag original to bottom
    plot(
      p_icb_plot + theme(plot.margin=unit(c(0,0,0,0),"in"))+
...
if(file.exists(save_file_phoenix_p) ){
  ...
  if(fromscratch | !file.exists(save_file_phoenix_png) ){
    p_plot <- readRDS(save_file_phoenix_p)
    try({invisible(dev.off())})
...
```{r terrier , eval=T, echo=F, results='asis',  include=T, message=F, cache=F, warning=F}
#terrier
...
if(file.exists(save_file_terrier_p)){
  ...
  if(fromscratch | !file.exists(save_file_terrier_png)){
    p_plot <- readRDS(save_file_terrier_p)
    plot_icews_height <- p_plot$sentence_count
    p_width=max(length(p_plot$actor_colors)*1.5,14)
    p_height=min(max((p_plot$data$y %>% unique() %>% length()) /2,6),80) #cap it at 60 inches long
    vertical_offset= (p_plot$data$y %>% range() %>% abs() %>% diff() %>% abs())/(p_plot$data$y %>% unique() %>% length())   #offsets are kind of complicated bc of the graph
    horizontal_offset= (p_plot$data$x %>% range() %>% abs() %>% diff() %>% abs())/length(p_plot$actor_colors)
...
if(file.exists(save_file_icews_p)){
  ...
  if(fromscratch | !file.exists(save_file_icews_png)){
    p_plot <- readRDS(save_file_icews_p)
...
if(file.exists(save_file_mids_p)){
  ...
  if(fromscratch | !file.exists(save_file_mids_png)){
    p_plot <- readRDS(save_file_mids_p)
    p_width=max(length(p_plot$actor_colors)*1.5,14)
    p_height=min(max(p_plot$sentence_count/2,6),12) #cap it at 49 inches long
...
```{r, eval=F , echo=F, results='asis',  include=T, message=F, cache=F, warning=F}
#Mids Incidents
if(file.exists(save_file_mids_incidents_p)){
  cat('\n\n##### MIDs Incidents \n\n')
  if(fromscratch | !file.exists(save_file_mids_incidents_png) ){
    p_plot <- readRDS(save_file_mids_incidents_p)
    p_width=max(length(p_plot$actor_colors)*1.5,14)
    p_height=min(max((p_plot$data$y %>% unique() %>% length()) /2,6),60) #cap it at 60 inches long
    vertical_offset= (p_plot$data$y %>% range() %>% abs() %>% diff() %>% abs())/(p_plot$data$y %>% unique() %>% length())   #offsets are kind of complicated bc of the graph
    horizontal_offset= (p_plot$data$x %>% range() %>% abs() %>% diff() %>% abs())/length(p_plot$actor_colors)
...
if(file.exists(save_file_ucdp_p)){
  ...
  if(fromscratch | !file.exists(save_file_ucdp_png) ){
    p_plot <- readRDS(save_file_ucdp_p)
    p_width=max(length(p_plot$actor_colors)*1.5,14)
    p_height=min(max((p_plot$data$y %>% unique() %>% length()) /2,6),60) #cap it at 60 inches long
    vertical_offset= (p_plot$data$y %>% range() %>% abs() %>% diff() %>% abs())/(p_plot$data$y %>% unique() %>% length())   #offsets are kind of complicated bc of the graph
    horizontal_offset= (p_plot$data$x %>% range() %>% abs() %>% diff() %>% abs())/length(p_plot$actor_colors)
