...
#icb_long_clean <- readRDS(file=paste0(here::here(), "//data_in//ICBe_V1_long_clean.Rds"))
#icb_wide_clean <- readRDS(file=paste0(here::here(), "//data_in//ICBe_V1_wide_clean.Rds"))
codings_long_agreed <- readRDS( paste0(here::here(), '//data_in//ICBe_V1_long_agreed.Rds'))
codings_wide_agreed <- readRDS(file=paste0(here::here(),'//data_in//ICBe_V1_wide_agreed.Rds'))


icb_crises <- read.csv(paste0(here::here(), "//data_in//icb1v14.csv")) %>% filter(crisno<=476 & crisno!=475 & !is.na(yrtrig)) #our data only go to 476 and 475 is broken
...
icbe_metro_plots <- function(crisis=196, prune_rare=T){
  ...
  print(crisis)
  #crisis=426
  if(!is.finite(crisno_name_years %>% filter(crisno==crisis) %>% pull(yrtrig))){return()}
  plot_title <- crisno_name_years %>% filter(crisno==crisis) %>% mutate(title="Crisis #" %>% paste0(crisno, " ", crisname, " (", yrtrig, "-",yrterm,")")) %>% pull(title)
  save_file <- paste0(here::here(), "/images/ggplots/p_metro_plot_", crisis, ".Rds")
  
  codings_wide_agreed_subset <- codings_wide_agreed %>% 
                                 ungroup() %>%
                                 dplyr::filter(crisno==crisis) %>%  #417 196 477
  ...
  units <-   codings_wide_agreed_subset %>% dplyr::select(sent=final_event_ordering, do_actor_a, interact_units) %>% na.omit()  %>% 
              ...
              mutate(do_actor_a = strsplit(as.character(do_actor_a), ";")) %>% unnest(do_actor_a) %>%
              mutate(IMAGE=paste0(here::here(), '/images/ggplots/symbols/',interact_units,'.png')) %>%
...
icb_crises$crisno %>% sapply(FUN=function(x) icbe_metro_plots(x))
...
icb_sentence_tables <- function(crisis=196){
  ...
  save_file <- paste0(here::here(), "/images/tables/sentence_tables/ft_sentence_table_",crisis,".Rds")
...
icb_crises$crisno %>% sapply(FUN=function(x) icb_sentence_tables(x))
...
dyadic_mid <- read_csv(paste0(here::here(), "//data_in//dyadic_mid_4.02.csv"))
...
mids_metro_plots <- function(crisis=196, prune_rare=T){
  ...
  #crisis=426
  plot_title <- crisno_name_years %>% filter(crisno==crisis) %>% mutate(title="Crisis #" %>% paste0(crisno, " ", crisname, " (", yrtrig, "-",yrterm,")")) %>% pull(title)
  save_mids_file <- paste0(here::here(), "/images/ggplots/p_mids_metro_plot_",crisis,".Rds")
...
icb_crises$crisno %>% sapply(FUN=function(x) mids_metro_plots(x))
...
midii <- readRDS(paste0(here::here(), '//data_in//MIDI_5.01.Rds'))
midip <- readRDS(paste0(here::here(), '//data_in//MIDIP_5.01.Rds'))
...
mids_incidents_metro_plots <- function(crisis=196, prune_rare=T){
  ...
  print(crisis)
  #crisis=426
  save_mids_incidents_file <- paste0(here::here(), "/images/ggplots/p_mids_incidents_metro_plot_",crisis,".Rds")
...
icb_crises %>% filter(yrtrig>=1990 & crisno %in% icb_crises$crisno) %>% pull(crisno) %>% unique()  %>% sapply(FUN=function(x) mids_incidents_metro_plots(x))
...
#icbdy_20 <- read_tsv(here::here(), "//data_in//icbdy_20.dat")
icbdy_20 <- readRDS(paste0(here::here(), '//data_in//icbdy_20.Rds'))
...
icb2v14 <- read_csv(paste0(here::here(), "//data_in//icb2v14.csv"))
...
icb_metro_plots <- function(crisis=196, prune_rare=T){
    ...
    print(crisis)
    save_icb_file <- paste0(here::here(), "/images/ggplots/p_icb_metro_plot_",crisis,".Rds")
...
icb_crises$crisno %>% sapply(FUN=function(x) icb_metro_plots(x))
...
load(paste0(here::here(), "//data_in//GEDEvent_v21_1.RData"))
ged211 <- GEDEvent_v21_1

#ged211 <- read_csv(paste0(here::here(), "/data_ignore/ged211.csv"))
...
ucdp_ged_metro_plots <- function(crisis=466, prune_rare=T){
    ...
    print(crisis)
    #crisis=426
    save_ucdp_incidents_file <- paste0(here::here(), "/images/ggplots/p_ucdp_metro_plot_",crisis,".Rds")
...
icb_crises %>% filter(yrtrig>=1989 & crisno %in% icb_crises$crisno ) %>% pull(crisno) %>% unique()  %>% sapply(FUN=function(x) ucdp_ged_metro_plots(x))
...
CAMEO_eventcodes <- read_tsv(paste0(here::here(), "//data_in//CAMEO.eventcodes.txt"), col_types='cc') %>% janitor::clean_names()
...
#CAMEO_eventcodes <- read_tsv(paste0(here::here(), "//data_in//CAMEO.eventcodes.txt")) %>% janitor::clean_names()
...
if(!file.exists(paste0(here::here(), "/data_ignore/icews_dataverse/dataverse_files/"))){
  stop("Missing 'UCDP Georeferenced Event Dataset (GED)'. See README.")
}

files <- list.files(path = paste0(here::here(), "/data_ignore/icews_dataverse/dataverse_files/"), full.names = T)
...
icews_metro_plots <- function(crisis=426, topactor=F, exclude='', savefile=NULL){
  ...
  #save_icews_file = paste0(here::here(), "/images/tables/sentence_tables/figures/p_icews_metro_plot_",crisis,"_paper.Rds")
  plot_title <- crisno_name_years %>% filter(crisno==crisis) %>% mutate(title="Crisis #" %>% paste0(crisno, " ", crisname, " (", yrtrig, "-",yrterm,")")) %>% pull(title)
  if(is.null(savefile)){
    savefile <- paste0(here::here(), "/images/ggplots/p_icews_metro_plot_",crisis,".Rds")
  }
  ...
  qcodes <- codings_long_agreed %>% dplyr::filter(crisno==crisis & variable_normalized %>% str_detect('actor') & value_normalized!='') %>% pull(value_normalized) %>% unique() %>% na.omit()
...
icb_crises %>% filter(yrtrig>=1995  & crisno %in% icb_crises$crisno) %>% pull(crisno) %>% unique()  %>% sapply(FUN=function(x) icews_metro_plots(x))
...
a <- readRDS(paste0(here::here(), '//data_in//PhoenixBLN-NYT_1980-2018.Rds'))
b <- dplyr::bind_rows(
  readRDS(paste0(here::here(), '//data_in//PhoenixBLN-SWB_1979-2019a.Rds')),
  readRDS(paste0(here::here(), '//data_in//PhoenixBLN-SWB_1979-2019b.Rds')),          
  readRDS(paste0(here::here(), '//data_in//PhoenixBLN-SWB_1979-2019c.Rds')))
c <- readRDS(paste0(here::here(), '//data_in//PhoenixFBIS_1995-2004.Rds'))
d <- readRDS(paste0(here::here(), '//data_in//PhoenixNYT_1945-2005.Rds'))
e <- readRDS(paste0(here::here(), '//data_in//PhoenixWSJ_1945-2006.Rds'))
...
phoenix_metro_plots <- function(crisis=196, topactor=F){
  ...
  #crisis=426
  plot_title <- crisno_name_years %>% filter(crisno==crisis) %>% mutate(title="Crisis #" %>% paste0(crisno, " ", crisname, " (", yrtrig, "-",yrterm,")")) %>% pull(title)
  save_phoenix_file <- paste0(here::here(), "/images/ggplots/p_phoenix_metro_plot_",crisis,".Rds")
...
icb_crises$crisno %>% sapply(FUN=function(x) phoenix_metro_plots(x))
...
if(!file.exists(paste0(here::here(), "/data_ignore/terrier/largegeolocateddata//"))){
  stop("Missing Terrier event data. See README.")
}


files <- list.files(path = paste0(here::here(), "/data_ignore/terrier/largegeolocateddata/"), pattern = ".tsv$", full.names = T)
...
#Subset reduce and save as parquet so it can be kept locally
terrier_subset <- list()
for(file in files){
  terrier_subset[[file]] <-   file %>% read_tsv(col_names=F, col_types=paste0(rep('c',50), collapse='')  )   %>% janitor::clean_names() %>% 
    dplyr::select(
      cameo_code=x3,
      src_country=x8,
      src_agent=x9,
...
terrier_clean_temp <- bind_rows(terrier_subset )  %>% janitor::clean_names() %>%
...
dim(terrier_clean_temp) #2,945,196
...
dim(terrier_clean) #2,049,759      21
...
terrier_metro_plots <- function(crisis=196, topactor=F){
  ...
  #crisis=426
  plot_title <- crisno_name_years %>% filter(crisno==crisis) %>% mutate(title="Crisis #" %>% paste0(crisno, " ", crisname, " (", yrtrig, "-",yrterm,")")) %>% pull(title)
  save_terrier_file <- paste0(here::here(), "/images/ggplots/p_terrier_metro_plot_",crisis,".Rds")
...
icb_crises %>% filter(yrtrig>=1977 & crisno %in% icb_crises$crisno) %>% pull(crisno) %>% unique()  %>% sapply(FUN=function(x) terrier_metro_plots(x))
